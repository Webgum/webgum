...
import {
  ...
  VectorOutput,
  WebgumContractAbi,
} from "../../contracts/WebgumContractAbi";
...
const AllProducts = ({ creatorVector, contract }: IAllProducts) => {
  ...
  useEffect(() => {
    async function getProjects() {
      if (creatorVector && contract) {
        let num_of_projects =
          parseFloat(creatorVector.current_ix.format()) * 1_000_000_000;
        ...
        for (let i = 0; i < num_of_projects; i++) {
          ...
          let resp = await contract.functions
            .get_project(creatorVector.inner[i])
            .get();
  ...
  return (
    <>
      <p>
        You have listed {totalProjects}{" "}
        {totalProjects == 1 ? "project" : "projects"}
      </p>
      <div className={styles.container}>
        {allProjects?.map((project) => (
          <div key={project.project_id.format()}>
